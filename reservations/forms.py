...
from .models import RezerwacjaSali, RezerwacjaPokoju, Pomieszczenie
...
class NewDormRoomReservationForm(forms.ModelForm):
    ...
    def clean(self):
        super(NewDormRoomReservationForm, self).clean()
        print(self.cleaned_data)
        data_od = self.cleaned_data.get('data_od')
        data_do = self.cleaned_data.get('data_do')
        if data_od < timezone.now() or data_do < timezone.now():
...
class ChangeDormRoomReservationStatusForm(forms.ModelForm):
